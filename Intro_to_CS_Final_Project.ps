...
def get_secondary_connections(network, user):
    ...
    if user not in network:
        return None
    for person in get_connections(network, user):
        for person2 in get_connections(network, person):
            if person2 not in secondary_connections:
                secondary_connections.append(person2)
...
def connections_in_common(network, user_A, user_B):
    ...
    for person in user_A_connections:
        if person in user_B_connections:
            number_connections += 1
    return number_connections

#print connections_in_common(network, "Walter", "John")
...
def path_to_friend(network, user_A, user_B, peep_path = None):
    if peep_path == None:
        peep_path = []
    if (user_A in network) and (user_B in network):
        if user_B in peep_path:
            return peep_path
        if user_A == user_B:
        ...
        if user_A in peep_path:
            ...
            for peep in get_connections(network, user_A):
                new_path = path_to_friend(network, peep, user_B, peep_path)
                if new_path is None:
                    del peep_path[-1]
                else:
                    peep_path = new_path
...
# Return:
#   The updated network with the connections added to a new user.
#   - If the user exists in the network, return network *UNCHANGED*

#print add_new_user_connections(network, 'Lola', ['Olive', 'Ollie', 'Freda', 'Mercedes'])
...
#print connections_in_common(net, "Alice", "Bob")
